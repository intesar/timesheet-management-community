...
public class ItemServiceTest extends AbstractTransactionalDataSourceSpringContextTests {
    ...
    protected String[] getConfigLocations() {
        return new String[]{"classpath:context/applicationContext-DAO.xml", "classpath:context/applicationContext-Services.xml", "classpath:context/applicationContext-Transactions.xml"};
    }
    ...
    protected void onSetUpInTransaction() throws Exception {
        ...
        jdbcTemplate.execute("INSERT INTO ITEM ( ID, NAME, DESCRIPTION, PRICE, TAX ) VALUES (2, 'TANDOORI CHICKEN', 'chicken grilled in a owen with chilly and spices', 5.99, .75)");
        jdbcTemplate.execute("INSERT INTO ITEM ( ID, NAME, DESCRIPTION, PRICE, TAX ) VALUES (3, 'BUTTER CHICKEN', 'chicken cooked in chilly and butter gravy', 5.99, .75)");
        jdbcTemplate.execute("INSERT INTO ITEM ( ID, NAME, DESCRIPTION, PRICE, TAX ) VALUES (4, 'BEEF KABAB', 'grilled beef baked on charkol', 5.99, .75)");
        jdbcTemplate.execute("INSERT INTO ITEM ( ID, NAME, DESCRIPTION, PRICE, TAX ) VALUES (5, 'LAMB BIRYANI', 'traditional hyderabadi rice cooked with lamb', 5.99, .75)");
        jdbcTemplate.execute("INSERT INTO ITEM ( ID, NAME, DESCRIPTION, PRICE, TAX ) VALUES (6, 'NAAN', 'baked flour', 5.99, .75)");
        jdbcTemplate.execute("INSERT INTO ITEM ( ID, NAME, DESCRIPTION, PRICE, TAX ) VALUES (7, 'lassi', 'curd chilled with sugar and ice', 2.99, .75)");
    ...
    public void testgetMatchingItems1() {
        String key = "LASSI";
        Collection<Item> items = itemService.getMatchingItems(key);
        assertTrue( items.size() == 1 );
    }
    public void testgetMatchingItems2() {
        String key = "CURD SUGAR";
        Collection<Item> items = itemService.getMatchingItems(key);
        assertTrue( items.size() == 1 );
    }
    
    public void testgetMatchingItems3() {
        String key = "3";
        Collection<Item> items = itemService.getMatchingItems(key);
        assertTrue( items.size() == 1 );
    }
    
    public void testgetMatchingItems4() {
        String key = "3 9";
        Collection<Item> items = itemService.getMatchingItems(key);
        assertTrue( items.size() == 6 );
    }
    
    public void testgetMatchingItems5() {
        String key = "3..9";
        Collection<Item> items = itemService.getMatchingItems(key);
        assertTrue( items.size() == 6 );
    }
    public void testgetMatchingItems6() {
        String key = "cHICKEN CHILLY";
        Collection<Item> items = itemService.getMatchingItems(key);
